...
import { Search, SlidersHorizontal, ArrowUpDown, ChevronLeft, ChevronRight, MoreHorizontal } from 'lucide-react';
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-200/80 bg-slate-50/60 text-[11px] font-semibold text-slate-700 select-none">
              <th className="py-2.5 pl-4 pr-1 w-7">
                <input
                  type="checkbox"
          ...
          <tbody className="divide-y divide-slate-100 text-xs">
            {paginatedItems.map((item) => {
              ...
              return (
                ...
                >
                  {/* Checkbox */}
                  <td className="py-2.5 pl-4 pr-1" onClick={(e) => toggleSelectItem(item.id, e)}>
                    <input
                      type="checkbox"
                      checked={isChecked}
                      onChange={() => {}}
